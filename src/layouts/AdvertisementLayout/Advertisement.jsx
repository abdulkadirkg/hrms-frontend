...
export default function Advertisement({ advertisement }) {
  ...
  const getAdvertisement = (advertisement) => {
    // console.log(favoriteItems.find((f) => f.advertisement.id === advertisement.id));
    return favoriteItems.find((f) => f.advertisement.id === advertisement.id);
  };
  return (
    <div>
      <li className="shadow-sm text-justify position-relative" key={advertisement.id}>
        <button onClick={() => handleAddFavorite(advertisement)} className="btn m-0 btn-add-favorites my-auto btn-sm py-0 px-1 small btn-outline-danger">
          {getAdvertisement(advertisement) ? <i className="fa my-auto fa-heart"></i> : <i className="far my-auto pt-1 fa-heart"></i>}
        </button>
